...
> 更新基线：`main` 的 v0.3.2 状态。本文用于后续对话快速恢复上下文；它描述的是**已经落地的代码**，不是下一阶段需求。

## 当前能力

- 本地 Meme 库：上传、缩略图、重复图片检测、搜索、标签筛选、随机查看、编辑、删除与分页加载。
- 桌面优先的原生 TypeScript 前端：瀑布流卡片、右侧详情面板、原图查看器、标签收纳/展开和窄窗口不溢出。
- AI 图片分析：对已上传 Meme 生成中文描述和 2 至 8 个标签建议；用户确认后才写入标签和可选描述。
- 网页内 API 设置：维护 AI 提供商、模型和当前视觉模型；内置 OpenAI、Qwen、DeepSeek 与自定义 OpenAI 兼容接口预设。

## 明确尚未实现

- 没有 `Template` / `MemeTemplate` 数据模型、数据库表、模板接口或前端模板选择器。
- 因而也没有“AI 从已有模板中匹配模板”的功能。已创建但尚未开发、且不会推送的隔离分支为 `codex/v0.3.3-ai-template-matching`；不要将其视为已具备模板能力。
- 尚未实现 AI 文案实验室、语义搜索、Meme 制作器、用户系统或云端存储。
...
app/
  ...
  models/          Meme、Tag、MemeAIAnalysis、AIProvider、AIModel
...
- `meme_ai_analyses`：模型名、AI 描述、标签建议 JSON、创建/确认时间。
...
如需恢复“AI 模板匹配”计划，先单独完成模板归类的基础层：Template ORM/迁移、Meme 的可空模板归属、Repository/Service/API、上传和编辑选择器与测试。之后才适合实现“AI 只能在已有模板中建议一个 ID 或 null”的增量；不要让 AI 自动创建或保存模板。
