...
> 更新基线：v0.4.0 实现状态（2026-07-30）。本文描述已经落地的代码，不是下一阶段需求。

## 当前能力

- 本地 Meme 库：上传、缩略图、全局重复图片检测、搜索、标签筛选、随机查看、编辑、删除与分页加载。
...
- AI 组级分析：一次请求按顺序读取完整图片组，生成一份中文描述、2 至 8 个标签建议和一个已有模板 ID 或 `null`；用户确认后才写入。
...
- 上传入口仍一次创建一个单图 Meme；后续图片只能从详情页逐张追加。
...
- `meme_ai_analyses` 保存一条完整 Meme 对应的一次组级建议快照。
...
确认接口继续在一个事务中应用用户选择的标签、描述和模板。未配置模型/密钥返回 503，超时返回 504，上游或结构化输出错误返回 502。
